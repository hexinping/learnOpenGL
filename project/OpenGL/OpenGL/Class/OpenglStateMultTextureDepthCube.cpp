...
bool OpenglStateMultTextureDepthCube::init(string vertFile, string fragFile)
{
	...
	genTexImage2D("resource/container2.png", GL_RGBA, 0, GL_TEXTURE1,GL_REPEAT, GL_LINEAR);
	genTexImage2D("resource/container2_specular.png", GL_RGBA, 0, GL_TEXTURE2, GL_REPEAT, GL_LINEAR);
	genTexImage2D("resource/matrix.jpg", GL_RGB, 0, GL_TEXTURE3, GL_REPEAT, GL_LINEAR);
	...
	*/
	...
	setInt(_shaderProgram, "material.diffuse", 1); // 这里的0就对应了前面的GL_TEXTURE0
	setInt(_shaderProgram, "material.specular", 2); // 这里的1就对应了前面的GL_TEXTURE1
	setInt(_shaderProgram, "material.emission", 3);
...
bool OpenglStateMultTextureDepthCube::isUseEBORender()
{
	return false;
}

void OpenglStateMultTextureDepthCube::rendeCommand()
{
...
int OpenglStateMultTextureDepthCube::getShaderIndex()
{
	return 18;
}


bool OpenglStateMultTextureDepthCube::isLihgtAction()
{
	return false;
}

int  OpenglStateMultTextureDepthCube::getPointLights()
{
	return 0;
}

bool OpenglStateMultTextureDepthCube::isShowLight()
{
	return false;
}


void OpenglStateMultTextureDepthCube::enableVertexAttribArray()
{
...
void OpenglStateMultTextureDepthCube::enableLightVertexAttribArray()
{
	GLint posLocation = _glUtils->getAttribLocation(_shaderProgram, "aPos");
